...
export default function DeleteModal({ config }) {
  ...
  let {
    entity,
    // Provide safe default for pages that don't specify labels
    deleteModalLabels = [],
  ...
  useEffect(() => {
    ...
    if (current) {
      const candidates = Array.isArray(deleteModalLabels) && deleteModalLabels.length
        ? deleteModalLabels
        : ['name', 'code', 'number', 'title'];
      let labels = candidates
        .map((x) => valueByString(current, x))
        .filter(Boolean)
        .join(' ');
      if (typeof transformDeleteLabel === 'function') {
        labels = transformDeleteLabel({ current, labels });
      }
