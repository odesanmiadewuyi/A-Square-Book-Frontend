...
export default function ProductCategoryListReport({ onReady }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (mounted) setError('Unable to load product categories.');
      } finally {
  ...
  return (
    <div style={{ background: '#fff', padding: 24, borderRadius: 10 }}>
      ...
      <div style={{ border: '1px solid #e2e8f0', borderRadius: 10, overflow: 'hidden' }}>
        ...
        {rows.length === 0 ? (
          <div style={{ padding: 14, fontSize: 12, color: '#64748b' }}>No categories found.</div>
        ) : (
          rows.map((category) => (
            <div
              key={category._id}
            ...
            >
              <div style={tableCell}>{category?.name || '-'}</div>
              <div style={tableCell}>{category?.description || '-'}</div>
            </div>
          ))
        )}
      </div>
