...
export default function InvoiceLocalReport({ id, onReady }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('');
      try {
        const resp = await request.read({ entity: 'invoice', id });
        const data = resp?.result || resp?.data || resp;
        if (mounted) setInvoice(data);
      } catch (err) {
        if (mounted) setError('Unable to load invoice report data.');
      } finally {
  ...
  if (!invoice) {
    return <div style={{ padding: 16 }}>Invoice not found.</div>;
  }
  ...
  const voucher =
    ...
    (invoice?.number ? formatVoucherNumber(invoice.number) : '') ||
    invoice?._id;
    ...
    <div style={{ background: '#fff', padding: 24, borderRadius: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 18 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 13, fontWeight: 600, color: '#0f172a' }}>Invoice Voucher</div>
