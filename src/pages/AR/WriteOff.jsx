...
export default function ARWriteOff() {
  ...
  const writeoffCode = (gl?.gl_ar_writeoff_postingcode || '').toString().trim();
  ...
  const effectiveWriteoff = (writeoffCode || newWriteoff || '').toString().trim();
  ...
  const onSubmit = async (values) => {
    try{
      if (!effectiveAR || !effectiveWriteoff) { message.warning('Configure AR Control and Write-Off posting codes'); return; }
      ...
      const { success } = await request.post({ entity: 'ar/writeoff', jsonData: payload });
      if (success){ message.success('Write-Off posted'); form.resetFields(); setOpenInvoices([]); setCustomerId(undefined); }
    } catch(e){}
  };

  return (
    <ErpLayout>
      <Card title='Accounts Receivable — Write-Off'>
        {(!effectiveAR || !effectiveWriteoff) && (
          <Alert type='warning' showIcon style={{ marginBottom: 12 }} message='Missing GL settings' description={
            <Space direction='vertical'>
              ...
              {!writeoffCode && (<Space><span>Write-Off Posting Code:</span><SelectAsync entity={'postingaccount'} outputValue={'postingcode'} displayLabels={['postingcode','name']} onChange={setNewWriteoff} /></Space>)}
              <Button onClick={async ()=>{
                try{
                  const settings = [];
                  if (!arControl && newARControl) settings.push({ settingKey:'gl_ar_control_postingcode', settingValue:newARControl });
                  if (!writeoffCode && newWriteoff) settings.push({ settingKey:'gl_ar_writeoff_postingcode', settingValue:newWriteoff });
        ...
        <Form form={form} layout='vertical' onFinish={onSubmit}>
          ...
          <div style={{ marginTop: 12 }}>
            <Button type='primary' htmlType='submit'>Post Write-Off</Button>
          </div>
        </Form>
      </Card>
    </ErpLayout>
