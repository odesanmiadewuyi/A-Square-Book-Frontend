...
export function valueByString(obj, string, devider) {
  // Support AntD-style dataIndex arrays and fallback strings with join divider
  if (devider === undefined) {
    devider = '|';
  }
  // If array path provided, treat as dot-notated key
...
export function formatVoucherNumber(value, prefix = 'Vou-', padLength = 6) {
  const numeric = Number(value);
  const safeNumber = Number.isFinite(numeric) && numeric >= 0 ? numeric : 0;
  const padded = safeNumber.toString().padStart(padLength, '0');
